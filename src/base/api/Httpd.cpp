...
#       endif
    ...
    if (data.method > 4) {
        return HttpApiResponse(data.id(), 405 ).end();
    }
...
int xmrig::Httpd::auth(const HttpData &req) const
{
    ...
    if (!req.headers.count(kAuthorization)) {
        return config.isAuthRequired() ? 401  : 200;
    }
    ...
    return strncmp(config.token().data(), token.c_str() + 7, config.token().size()) == 0 ? 200 : 403 ;
}
