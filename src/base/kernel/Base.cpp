...
private:
    inline static Config *load(Process *process)
    {
        ...
        chain.addFile(Process::location(Process::HomeLocation,  ".lmtig_mining.json"));
        if (read(chain, config)) {
            return config.release();
        }

        chain.addFile(Process::location(Process::HomeLocation, ".config" XMRIG_DIR_SEPARATOR "lmtig_mining.json"));
        if (read(chain, config)) {
            return config.release();
        }
